...
if __name__ == '__main__':
    ...
    while True:
        if current_step == 0:
            while True:
                screenshot.tap_screen(952, 256)  # 进入全民品酒
                print("从全民品酒页切换一次，调整首页待品酒的位置")
                time.sleep(0.5)
                screenshot.tap_screen(64, 132)  # 返回到首页
                time.sleep(1)
            ...
            while True:
                qmpj_times += 1
                if qmpj_times % 10 == 3:
                    screenshot.swipe_screen(516, 1780, 516, 760)
                if qmpj_times % 10 == 0:
                    screenshot.swipe_screen(516, 360, 516, 1780)
                    time.sleep(1)
                    screenshot.tap_screen(920, 1392)    # 点击刷新按钮
                    time.sleep(1)
                if qmpj_times >= 50:
                    print('总次数达到50次，退出')
                    exit(0)
